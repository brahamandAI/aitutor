...
const SERVER_HOST = 'tutorbuddy.co';
const APP_PATH = '/home/tutorbuddy/htdocs/tutorbuddy.co/tutorbuddy.co';
...
async function downloadEnvsFromGitHub() {
  ...
  try {
    ...
    const files = fs.readdirSync(cloneDir)
      .filter((file) => file.startsWith(dirName) && file.endsWith('.zip'));

    if (files.length === 0) {
      print.error(`No ${dirName}*.zip backups found in repository`);
      cleanup();
      return false;
    }
